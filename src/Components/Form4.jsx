...
const MultiStepForm = () => {
  ...
  const nextStep = () => {
    if (step === 1 && (!formData.name || !formData.email || !formData.phone || !formData.address || !formData.gender)) {
    ...
    if (step === 3) {
      if (
        (formData.employmentType === "salaried" && (!formData.company || !formData.PositionInCompany)) ||
        (formData.employmentType === "business" && (!formData.businessName || !formData.CatagoryOfBusiness))
      ) {
        alert("❌ Please fill in all required fields.");
        return;
      }
