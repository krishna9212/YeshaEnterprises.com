...
const MultiStepForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Car Loan Fields
    carType: storedData.carType || "",
    carModel: storedData.carModel || "",
    carYear: storedData.carYear || ""
});
  ...
  const nextStep = () => {
    if (step === 1) {
      ...
      if (step === 6 && (!formData.carType || !formData.carModel || !formData.carYear)) {
        alert("❌ Please fill in all car loan details.");
        return;
      }
...
{step === 6 && (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    ...
    <h2 className="text-3xl font-bold mb-4 text-gray-900 dark:text-white">Car Loan Details</h2>
    
    {/* Car Type Selection */}
    <select
      value={formData.carType}
      onChange={(e) => setFormData({ ...formData, carType: e.target.value })}
      className="w-full p-3 mb-3 border dark:border-gray-500 border-gray-300 rounded focus:ring-2 text-gray-700 dark:text-gray-200"
    >
